package com.suresh.microservices.account.api;

import lombok.extern.slf4j.Slf4j;
import org.springframework.web.bind.annotation.*;
import com.suresh.microservices.account.exceptions.AccountNotFoundException;
import com.suresh.microservices.account.model.Account;
...
public class Api {
	...
	public Account findByNumber(@PathVariable("number") String number) throws AccountNotFoundException {
		...
		return accounts.stream().filter(it -> it.getNumber().equals(number)).findFirst()
	...
	public Account findById(@PathVariable Integer id) throws AccountNotFoundException {
		log.info("Account.findAll()");
		return accounts.stream().filter(it -> it.getId().equals(id)).findFirst()
